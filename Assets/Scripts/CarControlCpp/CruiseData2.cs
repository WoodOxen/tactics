...
public class CruiseData2 : MonoBehaviour
{
    static int lastClosestWP = 0;
    private int NumofWP;
    public static double DistanceError;
    public static double Curvature;
    ...
    void Update()
    {
        ...
        {
            DistanceError = GetCruiseError(WaypointsModelAll[lastClosestWP].Position, WaypointsModelAll[tmpNum1].Position, transform.position);
        }
        else//即lastClosestWP-1和lastClosestWP号路标点是离车最近的两点  
        {
            DistanceError = GetCruiseError(WaypointsModelAll[tmpNum2].Position, WaypointsModelAll[lastClosestWP].Position, transform.position);
        }
        ...
        Curvature = GetCurvature(tmpNum1, tmpNum2, tmpNum3);
        //RadiusDisplay.GetComponent<TextMeshProUGUI>().text = "" + Curvature.ToString("#0.00");
        AngleError = WaypointsModelAll[lastClosestWP].Rotation.y - transform.eulerAngles.y;
    ...
    private WaypointsModel GetClosestWP(List<WaypointsModel> all, Vector3 myPosition)
    {
        ...
        {
            int j;
            for(int i = lastClosestWP - 5; i < lastClosestWP + 5; i++)
            {
                if (i < 0) j = i + NumofWP;
