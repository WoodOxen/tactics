...
//		if(hi > RoadDefKeysArray.Length-1){ hi = RoadDefKeysArray.Length-1; }
    ...
    public Vector3 GetSplineValue_getValue(float f,bool b = false){
		int i;
		int idx = - 1;
...
//		}
    ...
    public void GetSplineValue_Both_Catch_WayPoints(float f, out Vector3 tVect1, out Vector3 tVect2)
    {
        int i;
        int idx = -1;
        int mCount = GetNodeCount();
        ...
        if (mCount == 1)
        {
            ...
            int tmpi_1 = idx + 1;
            if (tmpi_1 == mNodes.Count) tmpi_1 = 0;
            //Vector4 wayPoint3 = new Vector4(idx, (tVect1 + mNodes[idx].pos).x, (tVect1 + mNodes[idx].pos).y, (tVect1 + mNodes[idx].pos).z);
            //Debug.Log(idx + "," + (mNodes[tmpi].pos + tVect1));
            WayPoints.way_points_pos.Add(tVect1);
            WayPoints.way_points_rot.Add(tVect2);
            WayPoints.node_belonging_to.Add(tmpi_1);
            //WayPoints.WayPoints.Add(wayPoint3);
            return;
        }
        ...
        int tmpi = idx + 1;
        if (tmpi == mNodes.Count) tmpi = 0;
        WayPoints.way_points_pos.Add(tVect1);
        WayPoints.way_points_rot.Add(tVect2);
        WayPoints.node_belonging_to.Add(tmpi);
    }
...
//		if(!bGizmoDraw){ return; }
		...
		for(i=0f;i<=1f;i+=step){
            ...
            //WayPoints.WayPoints.Add(new Vector4(1, cPos.x, cPos.y, cPos.z));
